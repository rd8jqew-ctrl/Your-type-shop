...
function save(db){
  ...
  if(storageReady && supabaseStore.enabled){
    return supabaseStore.queueSave(db);
  }
...
async function api(req,res,p){
 ...
 try{
  ...
  if(req.method==='GET'&&p==='/api/site-config')return send(res,200,{site:db.site,settings:db.settings},'application/json',origin);
  if(req.method==='PATCH'&&p==='/api/admin/site-config'){if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);const x=await body(req);if(x.hero!==undefined)db.site.hero=String(x.hero);if(x.announcement!==undefined)db.site.announcement=String(x.announcement);if(x.phone!==undefined||x.email!==undefined||x.social!==undefined){db.site.store={...(db.site.store||{})};if(x.phone!==undefined)db.site.store.phone=String(x.phone);if(x.email!==undefined)db.site.store.email=String(x.email);if(x.social!==undefined)db.site.store.social=String(x.social);}if(x.sections&&typeof x.sections==='object')db.site.sections=x.sections;if(x.sectionProducts&&typeof x.sectionProducts==='object')db.site.sectionProducts=x.sectionProducts;if(Array.isArray(x.colorPalette))db.site.colorPalette=x.colorPalette.map(v=>String(v).trim()).filter(Boolean);audit('site.update');save(db);return send(res,200,{ok:true,site:db.site},'application/json',origin)}
  ...
  if(req.method==='PATCH'&&p==='/api/admin/categories'){
    ...
    const x=await body(req);if(!Array.isArray(x.categories))return send(res,400,{error:'Categories must be an array'},'application/json',origin);
    db.site.categories=[...new Set(x.categories.map(v=>String(v).trim()).filter(Boolean))];audit('categories.update');await saveAndFlush(db);return send(res,200,{ok:true,categories:db.site.categories},'application/json',origin);
  }
  ...
  if(req.method==='POST'&&p==='/api/coupons/validate'){
   const x=await body(req),code=String(x.code||'').trim().toUpperCase(),subtotal=Math.max(0,priceNumber(x.subtotal));
   const c=db.coupons.find(v=>String(v.code||'').toUpperCase()===code&&v.active!==false);
   if(!c)return send(res,404,{error:'Invalid or inactive coupon'},'application/json',origin);
   if(c.expiresAt && new Date(c.expiresAt+'T23:59:59').getTime()<Date.now())return send(res,400,{error:'Coupon has expired'},'application/json',origin);
   if(subtotal<Number(c.minOrder||0))return send(res,400,{error:'Minimum order value is ₹'+Number(c.minOrder||0).toLocaleString('en-IN')},'application/json',origin);
   const discount=Math.min(subtotal,Math.max(0,subtotal*(Number(c.value||0)/100)));
   return send(res,200,{ok:true,code:c.code,discount},'application/json',origin);
  }
  ...
  if(req.method==='POST'&&p==='/api/orders'){
   ...
   const requestedCoupon=String(x.coupon||x.couponCode||'').trim().toUpperCase();
   if(requestedCoupon){const c=db.coupons.find(v=>String(v.code||'').toUpperCase()===requestedCoupon&&v.active!==false);if(!c)return send(res,400,{error:'Invalid or inactive coupon'},'application/json',origin);if(c.expiresAt&&new Date(c.expiresAt+'T23:59:59').getTime()<Date.now())return send(res,400,{error:'Coupon has expired'},'application/json',origin);if(subtotal<Number(c.minOrder||0))return send(res,400,{error:'Minimum order value is ₹'+Number(c.minOrder||0).toLocaleString('en-IN')},'application/json',origin);discount=Math.min(subtotal,Math.max(0,subtotal*(Number(c.value||0)/100)));couponCode=c.code;}
   const taxableSubtotal=Math.max(0,subtotal-discount),total=taxableSubtotal+shipping,id=orderId();
   const order={name:String(x.name).trim(),email:String(x.email).trim().toLowerCase(),phone:String(x.phone).trim(),address:String(x.address).trim(),city:String(x.city||'').trim(),pin:String(x.pin),items:requested,subtotal,discount,couponCode,taxableSubtotal,gstRate:0,gst:0,shipping,total,payment:String(x.payment||'cod').toLowerCase(),orderId:id,status:'New',date:new Date().toISOString(),userId:customer?.userId||null,verified:false,awb:'',courier:'',tracking_url:''};
  ...
  if(req.method==='POST'&&p==='/api/admin/upload'){
   ...
   let publicUrl='';
   if(supabaseStore.enabled && typeof supabaseStore.uploadMedia==='function'){
     publicUrl=await supabaseStore.uploadMedia(filename,buf,m[1]);
   }
   if(!publicUrl) {
     fs.writeFileSync(path.join(UPLOADS,filename),buf);
     publicUrl='/uploads/'+encodeURIComponent(filename);
   }
   audit('upload',{filename,type:m[1],size:buf.length,persistent:Boolean(publicUrl&&/^https?:/i.test(publicUrl))});
   await saveAndFlush(db);
   return send(res,201,{ok:true,filename,url:publicUrl,type:m[1],size:buf.length,persistent:/^https?:/i.test(publicUrl)},'application/json',origin);
  }
  ...
  if(req.method==='GET'&&p==='/api/admin/media'){
    if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);
    if(supabaseStore.enabled && typeof supabaseStore.listMedia==='function'){
      const media=await supabaseStore.listMedia();
  ...
  if(req.method==='DELETE'&&p.startsWith('/api/admin/media/')){
    if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);
    const name=path.basename(decodeURIComponent(p.slice('/api/admin/media/'.length)));
    if(supabaseStore.enabled && typeof supabaseStore.deleteMedia==='function'){
      const removed=await supabaseStore.deleteMedia(name);
      if(!removed)return send(res,404,{error:'Media not found'},'application/json',origin);
      audit('media.delete',{name,persistent:true});await saveAndFlush(db);return send(res,200,{ok:true},'application/json',origin);
    }
    const file=path.join(UPLOADS,name);if(!fs.existsSync(file)||!fs.statSync(file).isFile())return send(res,404,{error:'Media not found'},'application/json',origin);fs.unlinkSync(file);audit('media.delete',{name});await saveAndFlush(db);return send(res,200,{ok:true},'application/json',origin);
  }
