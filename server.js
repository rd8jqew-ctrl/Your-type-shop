...
const PERSISTENCE_REQUIRED=String(process.env.SUPABASE_REQUIRED|| (process.env.RENDER?'true':'false')).toLowerCase()==='true';
...
function normalizeImageRef(v){const s=String(v||'').trim();if(!s)return '';if(/^data:|^https?:|^\//i.test(s))return s;if(/^uploads\//i.test(s))return '/'+s;if(/^photos\//i.test(s))return '/'+s;return '/photos/'+s}
...
async function api(req,res,p){
 ...
 try{
  ...
  if(req.method==='POST'&&p==='/api/admin/change-password'){if(!auth(req,'admin'))return send(res,401,{error:'Unauthorized'},'application/json',origin);const x=await body(req);if(!verifyAdminPassword(x.currentPassword))return send(res,401,{error:'Current password is incorrect'},'application/json',origin);if(!strongPassword(x.newPassword))return send(res,400,{error:'New password must be 12–128 characters and include uppercase, lowercase, number and symbol'},'application/json',origin);adminAuth=saveAdminAuth(x.newPassword);db.sessions={};audit('admin.change-password');await saveAndFlush(db);return send(res,200,{ok:true},'application/json',origin)}
  if(req.method==='POST'&&p==='/api/admin/reset-password'){const x=await body(req);if(!ADMIN_RESET_TOKEN||x.resetToken!==ADMIN_RESET_TOKEN)return send(res,401,{error:'Invalid reset token'},'application/json',origin);if(!strongPassword(x.newPassword))return send(res,400,{error:'New password must be 12–128 characters and include uppercase, lowercase, number and symbol'},'application/json',origin);adminAuth=saveAdminAuth(x.newPassword);db.sessions={};await saveAndFlush(db);return send(res,200,{ok:true},'application/json',origin)}
  ...
  if(req.method==='POST'&&p==='/api/newsletter'){const x=await body(req),email=String(x.email||'').trim().toLowerCase();if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return send(res,400,{error:'Invalid email'},'application/json',origin);if(!db.newsletter.some(v=>v.email===email))db.newsletter.push({email,createdAt:new Date().toISOString()});await saveAndFlush(db);return send(res,201,{ok:true},'application/json',origin)}
  ...
  if(req.method==='POST'&&p==='/api/reviews'){const x=await body(req),name=String(x.product||'').trim(),title=String(x.title||'').trim(),text=String(x.text||'').trim(),rating=Math.max(1,Math.min(5,Math.floor(Number(x.rating||5))));const s=auth(req,'customer');if(!name||!title||!text||text.length>800)return send(res,400,{error:'Product, title and review text are required'},'application/json',origin);if(!s)return send(res,401,{error:'Please sign in to review'},'application/json',origin);if(!db.orders.some(o=>o.userId===s.userId&&o.status!=='Cancelled'&&o.items?.some(it=>it.name===name)))return send(res,403,{error:'You can review a product after purchasing it.'},'application/json',origin);const u=db.users.find(v=>v.id===s.userId);const r={id:crypto.randomUUID(),product:name,rating,title,text,name:u?.name||'Customer',verified:true,status:'pending',reply:'',createdAt:new Date().toISOString()};db.reviews.push(r);await saveAndFlush(db);return send(res,201,{ok:true,review:r},'application/json',origin)}
  ...
  if(req.method==='POST'&&p==='/api/admin/upload'){
   ...
   let mediaUrl='/uploads/'+encodeURIComponent(filename),storage='local';
   if(supabaseStore.enabled){
     const up=await supabaseStore.uploadMedia(buf,filename,m[1]);
     mediaUrl=up.url;storage='supabase';
   }else{
     fs.writeFileSync(path.join(UPLOADS,filename),buf);
   }
   audit('upload',{filename,type:m[1],size:buf.length,storage});await saveAndFlush(db);
   return send(res,201,{ok:true,filename,url:mediaUrl,type:m[1],size:buf.length,storage},'application/json',origin);
  }
  ...
  if(req.method==='DELETE'&&p.startsWith('/api/admin/media/')){
    ...
    if(supabaseStore.enabled){await supabaseStore.deleteMedia(name);audit('media.delete',{name,storage:'supabase'});await saveAndFlush(db);return send(res,200,{ok:true},'application/json',origin)}
    const safeName=path.basename(name),file=path.join(UPLOADS,safeName);if(!fs.existsSync(file)||!fs.statSync(file).isFile())return send(res,404,{error:'Media not found'},'application/json',origin);fs.unlinkSync(file);audit('media.delete',{name:safeName,storage:'local'});await saveAndFlush(db);return send(res,200,{ok:true},'application/json',origin);
  }
...
async function bootstrap(){
  try{
    if(PERSISTENCE_REQUIRED && !supabaseStore.enabled){
      throw new Error('Persistent storage is required in this deployment. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY before starting the server.');
    }
    if(supabaseStore.enabled){
      await supabaseStore.hydrateDb(db,{preserveDeletedIds:[...localDeletedProductIds]});
      // If this instance still has old local uploads, move them to durable Supabase Storage and rewrite their URLs.
      await supabaseStore.migrateLocalUploads(UPLOADS,db);
